...
def write_summary(rows: list[dict[str, object]], output_root: Path) -> None:
    ...
    with (output_root / "summary.md").open("w", encoding="utf-8") as fh:
        fh.write("# HardNet vector-field comparison\n\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare soft MLP and HardNet on a constrained vector field.")
    parser.add_argument("--seeds", type=str, default="0")
    parser.add_argument("--runtime-sec", type=float, default=3600.0, help="runtime budget per method and seed")
    ...
    parser.add_argument("--output-root", type=Path, default=Path("outputs/hardnet_vector_comparison_1h"))
